...
When('the user able to see autocomplete name', function(){
      signupPage.autoCompletenameTextBox().invoke('val').then(function(getName){
      expect(getName).to.equal(this.data.name)
    })
})

When('the user able to see autocomplete email address', function(){
    signupPage.autoCompleteEmailTextBox().invoke('val').then(function(getEmail){
...
Then('the user clicks on create account button and able to see account creation confirmation', function(){
    signupPage.getCreateAccount().click()
    cy.currentUrl().should('contain', Cypress.env('baseUrl').concat('account_created'))
    signupPage.getAccountHeading().should('have.text', signupPage.getAccountCreationTitle())
    //signupPage.getConfirmationMessage().should('have.text', signupPage.getconfirmationMessageHeading())
    signupPage.getMemberConfirmation().should('have.text', signupPage.getMemberConfirmationHeading())
    })
    
When('the user able to see login button in home screen', function(){
    signupPage.getAccountDetails().find(signupPage.getContinueButton()).click()
    cy.currentUrl().should('contain', Cypress.env('baseUrl'))
    signupPage.getLoginButton().should('have.attr', 'href').and('include', '/logout')
    signupPage.getDeleteButton().should('have.attr', 'href').and('include', '/delete_account')
    })
